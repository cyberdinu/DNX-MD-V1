...
import { Handler, Callupdate, GroupUpdate } from './date/index.js';
...
async function downloadSessionData() {
    ...
    const sessdata = config.SESSION_ID.split("DNX-MD~")[1];
...
async function start() {
    try {
        ...
        console.log(`🤖 DNX-MD using WA v${version.join('.')}, isLatest: ${isLatest}`);
        
        const Matrix = makeWASocket({
            version,
            logger: pino({ level: 'silent' }),
            printQRInTerminal: useQR,
            browser: ["DNX-MD", "safari", "3.3"],
            auth: state,
            getMessage: async (key) => {
                ...
                return { conversation: "DNX-MD whatsapp user bot" };
            }
        });
...
*Thanks for using DNX-MD 🚩* 

> Join WhatsApp Channel: ⤵️  
https://whatsapp.com/channel/0029Vb0pFMeGOj9r28xCQJ0w
    ...
    try {
        ...
        if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_STATUS_SEEN) {
            ...
            if (config.AUTO_STATUS_REPLY) {
                const customMessage = config.STATUS_READ_MSG || '✅ Auto Status Seen Bot By DNX-MD';
